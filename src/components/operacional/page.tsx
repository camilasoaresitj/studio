...
import type { GetTrackingInfoOutput } from '@/ai/flows/get-tracking-info';
...
export default function OperacionalPage() {
  ...
  const handleFetchNewBooking = async (bookingNumberToFetch: string) => {
      ...
      if (response.success) {
          ...
          // Check if a shipment with this booking or master bill number already exists
          ...
          if (existingShipmentIndex > -1) {
              // Merge with existing shipment
              const existingShipment = shipments[existingShipmentIndex];
              updatedShipment = {
                  ...existingShipment,
                  ...shipmentDetails, // This will overwrite fields like vesselName, voyageNumber, etd, eta, milestones
                  id: existingShipment.id, // Ensure original ID is preserved
              ...
              updatedShipment = {
                  ...
                  charges: [],
                  details: { cargo: '', transitTime: '', validity: '', freeTime: '' },
                  overseasPartner: { id: 0, name: 'Parceiro a definir', roles: { fornecedor: true } },
                  ...shipmentDetails,
              } as Shipment;
          ...
          updateShipment(updatedShipment); // Save all changes to local storage
          setNewBookingNumber('');
      } else {
          toast({
              variant: 'destructive',
