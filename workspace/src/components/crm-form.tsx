...
export function CrmForm() {
  ...
  async function onCampaignSubmit(values: z.infer<typeof campaignFormSchema>) {
    ...
    const partnersData = getPartners(); // Fetch fresh data
    const quotesData = getInitialQuotes(); // Fetch fresh data
    const response = await runCreateEmailCampaign(values.instruction, partnersData, quotesData);
    if (response.success) {
