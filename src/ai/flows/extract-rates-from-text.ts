...
**CRITICAL RULES:**
1.  **MANDATORY FIELDS**: You MUST NOT generate a rate object unless you can find a specific **origin**, **destination**, and **rate value** in the text. If any of these three are missing, you must ignore that line/rate completely.
2.  **MULTIPLE DESTINATIONS**: If a rate applies to multiple destinations (e.g., "SHA to SAN/RIO"), create a separate JSON object for each destination.
3.  **SLASH-SEPARATED RATES**: If you encounter a rate with three values separated by slashes (e.g., \`USD 6013/6226/6226\`), you MUST interpret this as rates for three different container types in the order: **20'GP**, **40'GP**, and **40'HC**. Create a separate JSON object for each of these rates.
4.  **EXPLICIT CONTAINER RATES**: If a rate has multiple prices for different containers explicitly named (e.g., "20GP/40HC: 2500/4800"), create a separate object for each container.
5.  **BRAZILIAN PORTS**: If the text mentions "brazil base ports" or "brazilian ports", you MUST generate separate rate entries for each of the main Brazilian ports: Santos, Paranaguá, Itapoá, and Rio Grande.
